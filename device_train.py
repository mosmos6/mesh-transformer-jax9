...
if __name__ == "__main__":
    ...
    with jax.sharding.Mesh(devices, ('dp', 'mp')):
        ...
        if initial_ckpt_state_path:
            ...
            start = time.time()
            #network.state = read_ckpt(network.state, initial_ckpt_state_path, shards_in=4, shards_out=devices.shape[1], load_opt=(not args.fresh_opt))
            network.state = read_ckpt(network.state, initial_ckpt_state_path, shards_in=4, shards_out=8, load_opt=(not args.fresh_opt))
